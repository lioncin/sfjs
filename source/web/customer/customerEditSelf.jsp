...
<head>
<title><c:choose><c:when test="${entity.id > 0}"><f:message key="cm.edit"/></c:when><c:otherwise><f:message key="ss.register.title"/></c:otherwise></c:choose></title>
...
<div class="row">
  <div class="col-md-12">
    <h3 class="page-header"><c:choose><c:when test="${entity.id > 0}"><f:message key="cm.edit"/></c:when><c:otherwise><f:message key="ss.register.title"/></c:otherwise></c:choose></h3>
  </div>
</div>
...
<input type="hidden" id="licenseFileData" name="licenseFileData" <%if (licenseAttachment != null) {%>value="id_<%=licenseAttachment.getId()%>"<%} %>>
<input type="hidden" id="taxFileData" name="taxFileData" <%if (taxAttachment != null) {%>value="id_<%=taxAttachment.getId()%>"<%} %>>
<input type="hidden" id="businessCfData" name="businessCfData" <%if (businessCfAttachment != null) {%>value="id_<%=businessCfAttachment.getId()%>"<%} %>>
<input type="hidden" id="qfProveData" name="qfProveData" <%if (qfProveAttachment != null) {%>value="id_<%=qfProveAttachment.getId()%>"<%} %>>
...
<input type="hidden" id="otherData" name="otherData" <%if (otherAttachment != null) {%>value="id_<%=otherAttachment.getId()%>"<%} %>>
      ...
      <div class="panel-heading"><c:choose><c:when test="${entity.id > 0}"><f:message key="cm.edit"/></c:when><c:otherwise><f:message key="cm.register"/></c:otherwise></c:choose></div>
        <div class="panel-body">
          ...
          <div id="changePwdDiv" style="display:none;">
            <div class="form-group">
              <label class="col-md-3 control-label"><span class="required">*</span><f:message key="cm.user.originalPwd"/>:</label>
              <div class="col-md-9"><input type="password" name="oldPassword" class="form-control m"/></div>
            </div>
            <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-md-9">
              ...
              <div>
                ...
                <a href="javascript:void(0);" onclick="javascript:deleteFile(this, 'licenseFileData');" title="<f:message key="cm.delete"/>"><span class="glyphicon glyphicon-remove"></span></a>
              </div>
              <%} %>
              <input type="file" id="licenseFile" name="licenseFile" class="upload"/>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-md-9">
              ...
              <div>
                ...
                <a href="javascript:void(0);" onclick="javascript:deleteFile(this, 'taxFileData');" title="<f:message key="cm.delete"/>"><span class="glyphicon glyphicon-remove"></span></a>
              </div>
              <%} %>
              <input type="file" id="taxFile" name="taxFile" class="upload"/>
            </div>
          </div>    
          <div class="form-group">
            ...
            <div class="col-md-9">
              ...
              <div>
                ...
                <a href="javascript:void(0);" onclick="javascript:deleteFile(this, 'businessCfData');" title="<f:message key="cm.delete"/>"><span class="glyphicon glyphicon-remove"></span></a>
              </div>
              <%} %>
              <input type="file" id="businessCertificate" name="businessCertificate" class="upload"/>
            </div>
          </div>    
          <div class="form-group">
            ...
            <div class="col-md-9">
              ...
              <div>
                ...
                <a href="javascript:void(0);" onclick="javascript:deleteFile(this, 'qfProveData');" title="<f:message key="cm.delete"/>"><span class="glyphicon glyphicon-remove"></span></a>
              </div>
              <%} %>
              <input type="file" id="qualificationProve" name="qualificationProve" class="upload"/>
            </div>
          </div>
            <%-- <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-md-9">
              ...
              <div>
                ...
                <a href="javascript:void(0);" onclick="javascript:deleteFile(this, 'otherData');" title="<f:message key="cm.delete"/>"><span class="glyphicon glyphicon-remove"></span></a>
              </div>
              <%} %>
              <input type="file" id="other" name="other" class="upload"/>
            </div>
          </div>
          <div class="action">
            <input type="button" id="submitBtn" class="btn btn-default" onclick="javascript:doSubmit();" value="<f:message key="cm.submit"/>"/>&nbsp;<input type="button" class="btn btn-default" onclick="javascript:resetForm();" value="<f:message key="cm.reset"/>"/>
          </div>
       </div>
    </div>
  </div>
...
function displayFile() {
    if ($("#licenseDown").length > 0) {
        $("#licenseFile").attr({ style: "display:none" });
    }
    if ($("#taxDown").length > 0) {
        $("#taxFile").attr({ style: "display:none" });
    }
    if ($("#businessCertificateDown").length > 0) {
        $("#businessCertificate").attr({ style: "display:none" });
    }
    if ($("#qualificationProveDown").length > 0) {
        $("#qualificationProve").attr({ style: "display:none" });
    }
    if ($("#otherDown").length > 0) {
        $("#other").attr({ style: "display:none" });
    }
}

function deleteFile(elem, deleteData){
  if (confirm('<f:message key="cm.confirm.delete"/>')) {
    $(elem).parent().next("input[type='file']").removeProp('style');
    $(elem).parent().remove();
    $('#'+deleteData).val('');
  }
}
...
function doSubmit() {
    if (checkRequired("companyName") & checkRequired("licenseNo") & checkRequired("legalRep") & checkRequired("address") & checkRequired("orgCode")
        ...
        submitForm('<c:url value="/organization/modify"/>');
    }
}
