...
<form class="form-horizontal" name="mainForm" id="mainForm" action="<c:url value="search"/>" method="post">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <div class="form-group">
          ...
          <label class="col-md-2 control-label"><f:message key="activity.status"/></label>
          <div class="col-md-4"><f:masterCode name="status" css="form-control m" category="activity.status" code="${l_searchParam.filters['status']}" firstOption="--"/></div>
        </div>
        ...
        <div class="dataTable_wrapper">
          <table class="table table-striped table-bordered table-hover">
            ...
            <tr>
              ...
              <td class="set"><%if(groupId == 103) {%>
              <%if(!"A".equals(row.getStatus())) {%><a href="javascript:doEdit('<%=id%>');" title="<f:message key="cm.edit"/>"><span class="glyphicon glyphicon-edit"></span></a><%} %>
              <%} %>&nbsp;<a href="javascript:doSingleDelete('<%=id%>');" title="<f:message key="cm.delete"/>"><span class="glyphicon glyphicon-remove"></span></a></td>
            </tr>
            <%
                  }
              } else {
...
function doView(id) {
    submitForm('<c:url value="/activity/view"/>/' + id);
}
