    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <div class="form-group">
          ...
          <div class="col-md-4"><f:masterCode name="crStatus" css="form-control m" category="customer.status" code="${l_searchParam.filters['crStatus']}" firstOption="--"/></div>
        </div>
        ...
        <div class="dataTable_wrapper">
          <table class="table table-striped table-bordered table-hover">
            ...
            <tr>
              ...
              <th><f:message key="cm.customer.phone"/></th>
              <th><f:message key="cm.customer.type"/></th>
            ...
            <tr>
              ...
              <td><f:masterCode code="customer.type.${row.type}"/></td>
              <td class="set">
                <c:choose>
                <c:when test="${row.status == 'A'}">
                <a href="javascript:changeStatus('${row.id}', 'D');" title="<f:message key="examine.approve.result.d"/>"><span class="glyphicon glyphicon-remove"></span></a>
                </c:when>
                <c:when test="${row.status == 'P'}">
                <a href="javascript:changeStatus('${row.id}', 'A');" title="<f:message key="examine.approve.result.y"/>"><span class="glyphicon glyphicon-ok"></span></a>
                </c:when>
                <c:when test="${row.status == 'D'}">
                <a href="javascript:changeStatus('${row.id}', 'A');" title="<f:message key="examine.approve.result.a"/>"><span class="glyphicon glyphicon-ok"></span></a>
                </c:when>
                <c:when test="${row.status == 'R'}">
                <a href="javascript:changeStatus('${row.id}', 'A');" title="<f:message key="examine.approve.result.y"/>"><span class="glyphicon glyphicon-ok"></span></a>
                </c:when>
                </c:choose>
              </td>
...
function doClear() {
    ...
    $('#crStatus option:eq(0)').attr('selected','selected');
}
//---------------- /CRUD ----------------

function doView(id) {
    submitForm('<c:url value="/member/view"/>/' + id);
}

function changeStatus(id, status) {
    if (confirm('<f:message key="examine.approve.affirm"/>')) {
        submitForm('<c:url value="/member/status"/>?id=' + id + '&result=' + status);
    }
}
</script>
</body>
