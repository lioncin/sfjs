...
<head>
<title><f:message key="lm.convention.statistics.search"/></title>
...
<div class="row">
  <div class="col-md-12">
    <h3 class="page-header"><f:message key="ce.statistics.conference.manage"/></h3>
  </div>
</div>

<form class="form-horizontal" role="form" name="mainForm" id="mainForm" action="<c:url value="/organization/statistics/conference/search"/>" method="post">
...
function doView(id) {
    submitForm('<c:url value="/organization/statistics/conference/view"/>/' + id);
}

function doEdit(id) {
  submitForm('<c:url value="/organization/statistics/conference/edit"/>/' + id);
}

function doSingleDelete(id) {
    if (confirm('<f:message key="cm.confirm.delete"/>')) {
        submitForm('<c:url value="/organization/statistics/delete"/>?id=' + id + '&fromView=conference');
    }
}

function doDelete() {
    if ($('input[name="selector"]').is(':checked') && confirm('<f:message key="cm.confirm.delete"/>')) {
        submitForm('<c:url value="/organization/statistics/delete?fromView=conference"/>');
    }
}
</script>
</body>
