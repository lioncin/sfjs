...
<script>
function doEdit() {
  submitForm('<c:url value="/organization/statistics/exhibition/edit"/>/' + '${entity.id}');
}
function doDelete() {
  if (confirm('<f:message key="cm.confirm.delete"/>')) {
    submitForm('<c:url value="/organization/statistics/delete"/>/?id=${entity.id}&fromView=exhibition');
  }
}
function doCancel() {
    submitLink('<c:url value="/organization/statistics/exhibition/search"/>');
}
</script>
</body>
</html>